...
class PlotPosteriorData(object):
    ...
    def plot_posterior_rwe(self, final=True, chainidx=0):
        ...
        X, Y = np.meshgrid(xedges, yedges)
        print(3)
        axes.plot(targets.targets[2].obsdata.x, targets.targets[2].obsdata.y, 'o', color='w', markersize=2)
        axes.plot(x_rwe, _y_rwe, color='w', linewidth=1)
        print(4)
